...
public sealed class BusinessUnitResolveHelper
{
    ...
    public async Task<string> ResolveAsync(string division)
    {
        ...
        try
        {
            var businessUnit = await _businessUnitRepository.GetBusinessUnitByDivisionAsync(
                division
            );
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error resolving business unit for Division: {Division}",
                division
            );
            throw;
        }
    ...
    public async Task<BusinessUnitConfig> GetBusinessUnitConfigAsync(string businessUnitCode)
    {
        ...
        try
        {
            var businessUnit = await _businessUnitRepository.GetBusinessUnitByCodeAsync(
                businessUnitCode
            );
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error getting business unit config for: {Code}",
                businessUnitCode
            );
            throw;
        }
    ...
    public async Task<List<BusinessUnitConfig>> GetAllBusinessUnitsAsync()
    {
        try
        {
            ...
            var configs = businessUnits
                .Select(bu => new BusinessUnitConfig
                {
    ...
    public string BuildConnectionString(string databaseName)
    {
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error building connection string for database: {Database}",
                databaseName
            );
            throw;
        }
