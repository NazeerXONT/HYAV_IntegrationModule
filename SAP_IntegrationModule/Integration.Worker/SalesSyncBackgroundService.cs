        ...
        using (LogContext.PushProperty("Environment", _configuration["Environment"] ?? "Unknown"))
        {
            ...
            if (!enableSync)
            {
                _logger.LogInformation(
                    "Customer Sync is disabled via configuration. Stopping service."
                );
                return;
            }
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                try
                {
                    ...
                    _logger.LogInformation(
                        "Customer Sync Background Service waiting for {Delay} minutes before next cycle.",
                        delay.TotalMinutes
                    );
                ...
                {
                    _logger.LogInformation(
                        "Customer Sync Background Service cancellation requested. Stopping."
                    );
                    break;
                }
            }
        ...
        while (true)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (retryCount > maxRetries)
                {
                    _logger.LogError(
                        ex,
                        "Customer sync failed after {RetryCount} attempts. Giving up.",
                        maxRetries + 1
                    );
                    throw;
                }
                ...
                _logger.LogWarning(
                    ex,
                    "Customer sync failed on attempt {Attempt}/{MaxRetries}. Retrying in 30 seconds...",
                    retryCount,
                    maxRetries + 1
                );
